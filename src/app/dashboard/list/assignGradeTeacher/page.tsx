...
const AssignGradeTeacher = () => {
  ...
  const columns = [
    ...
    { header: "Payment Status", accessor: "paymentStatus", className: "py-2 px-3 hidden md:table-cell" },
    ...
    <tr key={student._id} className={`text-left text-gray-500 text-md even:bg-slate-50 hover:bg-blue-50 ${selectedStudent === student._id ? 'bg-blue-100' : ''}`}>
      ...
      <td className="py-3 px-3 whitespace-nowrap hidden md:table-cell">
        <span className={`ml-2 px-3 py-1 rounded-full text-sm font-semibold ${
           student.paymentStatus === "success" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 mb-8">
        ...
        <div className="flex justify-end gap-2">
          <button
            type="submit"
            disabled={updating}
            className="bg-primary hover:bg-transparent border border-primary hover:text-primary text-white transition px-4 py-1 rounded-md"
          >
