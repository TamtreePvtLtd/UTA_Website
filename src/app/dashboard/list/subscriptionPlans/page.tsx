        ...
        <div className="py-6 px-4 md:px-2 bg-gray-50 mt-4">
          <div className="flex justify-center mb-8">
            <div className="bg-white shadow-md rounded-full p-1 flex">
              <button onClick={() => setBillingCycle("freeTrial")} className={`px-6 py-2 rounded-full font-medium transition ${billingCycle === "freeTrial" ? "bg-primary text-white" : "text-gray-600 hover:text-primary"}`}>Free Trial</button>
              <button onClick={() => setBillingCycle("monthly")} className={`px-6 py-2 rounded-full font-medium transition ${billingCycle === "monthly" ? "bg-primary text-white" : "text-gray-600 hover:text-primary"}`}>Monthly</button>
              <button onClick={() => setBillingCycle("yearly")} className={`px-6 py-2 rounded-full font-medium transition ${billingCycle === "yearly" ? "bg-primary text-white" : "text-gray-600 hover:text-primary"}`}>Yearly</button>
            </div>
          </div>
            ...
            {plans.filter((plan) => billingCycle === "freeTrial" ? plan.value === "freeTrial" : plan.value !== "freeTrial").map((plan) => (
              <div key={plan._id} className={`bg-white p-6 rounded-xl shadow hover:shadow-lg transition duration-300 ${billingCycle === "freeTrial" ? "w-full max-w-sm" : ""}`}>
                ...
                <button
                  ...
                  disabled={loading}
                  className={`w-full mt-2 py-2 text-white rounded ${selectedPlan === plan.value ? 'bg-primary' : 'bg-primary hover:bg-amber-600'} ${loading ? 'opacity-60' : ''}`}
                >
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg max-w-md w-full">
              ...
              <div className="flex justify-between gap-2 mt-4">
                <button onClick={() => handleConfirmPayment()} disabled={loading} className="px-4 py-2 bg-primary text-white rounded disabled:opacity-50">
